...
import bluePrintManager, {
  BPNodeLayoutType,
} from "../../../manager/BluePrintManager.ts";
...
export const BPNodeContainer: React.FC<BPNodeContainerProps> = React.memo(
  ({ layout }) => {
    ...
    useEffect(() => {
      ...
      if (prevNodeController) nodeConfig = prevNodeController.getConfig();
      else if (bpNodeConfigMap[layout.id!])
        nodeConfig = bpNodeConfigMap[layout.id!];
      else nodeConfig = ncIns.getNodeInfo(layout.id!);
